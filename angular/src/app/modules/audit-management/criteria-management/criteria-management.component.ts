          ...
          if (rsValid.success) {
            if (rsTailoring.result.length > 0) {
              ...
              abp.message.confirm(`<div style="display: flex; flex-direction: column; align-items: stretch; overflow-y: auto; max-height: 500px;">${message}</div>`, "",
                (result: boolean) => {
                  if (result) {
                    concat(this.processCriteriaService.RemoveCriteriaFromTailoring(node.id),this.processCriteriaService.delete(node.id))
    ...
    this.processCriteriaService.GetAllProcessTailoringContain(node.item.id).subscribe(rs => {
      if (rs.result.length > 0) {
        ...
        ${message}
          ...
          (result: boolean) => {
            if (result) {
              concat(this.processCriteriaService.RemoveCriteriaFromTailoring(node.item.id),this.processCriteriaService.DeActive(node.item.id))
                .pipe(catchError(this.processCriteriaService.handleError)).subscribe(() => {
