...
import {
  ...
  ReviewInternRetroHisotyDto,
} from "./../../../../../service/model/delivery-management.dto";
  ...
  implements OnInit
{
  ...
  Resource_TabPool_ViewHistory =
    PERMISSIONS_CONSTANT.Resource_TabPool_ViewHistory;
  Resource_TabPool_CreatePlan =
    PERMISSIONS_CONSTANT.Resource_TabPool_CreatePlan;
  Resource_TabPool_EditPlan = PERMISSIONS_CONSTANT.Resource_TabPool_EditPlan;
  Resource_TabPool_AddTempProject =
    PERMISSIONS_CONSTANT.Resource_TabPool_AddTempProject;
  Resource_TabPool_ConfirmPickEmployeeFromPoolToProject =
    PERMISSIONS_CONSTANT.Resource_TabPool_ConfirmPickEmployeeFromPoolToProject;
  ...
  Resource_TabPool_ConfirmOut =
    PERMISSIONS_CONSTANT.Resource_TabPool_ConfirmOut;
  Resource_TabPool_CancelMyPlan =
    PERMISSIONS_CONSTANT.Resource_TabPool_CancelMyPlan;
  Resource_TabPool_CancelAnyPlan =
    PERMISSIONS_CONSTANT.Resource_TabPool_CancelAnyPlan;
  Resource_TabPool_EditTempProject =
    PERMISSIONS_CONSTANT.Resource_TabPool_EditTempProject;
  Resource_TabPool_Release = PERMISSIONS_CONSTANT.Resource_TabPool_Release;
  Resource_TabPool_UpdateSkill =
    PERMISSIONS_CONSTANT.Resource_TabPool_UpdateSkill;
  Resource_TabPool_ViewUserStarSkill =
    PERMISSIONS_CONSTANT.Resource_TabPool_ViewUserStarSkill;
  Resource_TabPool_EditNote = PERMISSIONS_CONSTANT.Resource_TabPool_EditNote;
  Resource_TabPool_ProjectDetail =
    PERMISSIONS_CONSTANT.Resource_TabPool_ProjectDetail;
  Projects_OutsourcingProjects_ProjectDetail_TabWeeklyReport =
    PERMISSIONS_CONSTANT.Projects_OutsourcingProjects_ProjectDetail_TabWeeklyReport;
  ...
  Projects_ProductProjects_ProjectDetail_TabWeeklyReport =
    PERMISSIONS_CONSTANT.Projects_ProductProjects_ProjectDetail_TabWeeklyReport;
  Projects_ProductProjects_ProjectDetail_TabWeeklyReport_View =
    PERMISSIONS_CONSTANT.Projects_ProductProjects_ProjectDetail_TabWeeklyReport_View;
  Projects_TrainingProjects_ProjectDetail_TabWeeklyReport =
    PERMISSIONS_CONSTANT.Projects_TrainingProjects_ProjectDetail_TabWeeklyReport;
  Projects_TrainingProjects_ProjectDetail_TabWeeklyReport_View =
    PERMISSIONS_CONSTANT.Projects_TrainingProjects_ProjectDetail_TabWeeklyReport_View;
    ...
    this.subscription.push(
      this.availableRerourceService
        ...
        .subscribe((data) => {
          ...
          let listEmail = this.availableResourceList.map(
            (rs) => rs.emailAddress
          );
  ...
  public isAllowCancelPlan(creatorUserId: number) {
    if (
      this.permission.isGranted(
        this.DeliveryManagement_ResourceRequest_CancelMyPlanOnly
      )
    ) {
      if (
        this.permission.isGranted(
          this.DeliveryManagement_ResourceRequest_CancelAnyPlanResource
        )
      ) {
        return true;
    ...
    let ref = this.dialog.open(UpdateUserSkillDialogComponent, {
      ...
      data: {
        ...
        viewStarSkillUser: this.permission.isGranted(
          this.Resource_TabPool_ViewUserStarSkill
        ),
        id: user.userId,
        fullName: user.fullName,
        note: note,
  ...
  onSelectChange(id) {
    if (this.selectedSkillIdCr.includes(id)) {
      this.selectedSkillIdCr = this.selectedSkillIdCr.filter(
        (res) => res != id
      );
    ...
    this.unSelectedSkill = this.listSkills.filter(
      (item) => !this.selectedSkillId.includes(item.id)
    );
    this.selectedSkill = this.listSkills.filter((item) =>
      this.selectedSkillId.includes(item.id)
    );
    ...
    abp.message.confirm(
      ...
      (result: boolean) => {
        if (result) {
          this.subscription.push(
            this.availableRerourceService
              .CancelPoolResourcePlan(projectUser.id)
              .pipe(catchError(this.availableRerourceService.handleError))
              .subscribe((rs) => {
                this.refresh();
  ...
  confirm(plan, userId, userName) {
    ...
    this.projectUserService
      .GetAllWorkingProjectByUserId(userId)
      .subscribe((data) => {
  ...
  cancelResourcePlan(user) {
    abp.message.confirm(
      `Cancel plan for user <strong>${
        user.fullName
      }</strong> <strong class = "${
        user.allocatePercentage > 0 ? "text-success" : "text-danger"
      }">
      ${
        user.allocatePercentage > 0 ? "Join project" : "Out project"
      }</strong>?`,
      "",
      (result: boolean) => {
        if (result) {
  ...
  public GetRetroAndReviewInternHistories(emails: string[]) {
    this.availableRerourceService
      .GetTimesheetOfRetroReviewInternHistories({
        emails: emails,
      } as RetroReviewInternHistoriesDto)
      .subscribe(
        (rs) => {
          this.reviewInternRetroHisoties = rs.result;
          this.availableResourceList.forEach((x) => {
            x.avgPoint = this.reviewInternRetroHisoties.find(
  ...
  viewProjectDetail(project) {
    ...
    if (project.projectType == 5) {
      ...
      routingToUrl =
        this.permission.isGranted(
          this.Projects_OutsourcingProjects_ProjectDetail_TabWeeklyReport
        ) &&
        this.permission.isGranted(
