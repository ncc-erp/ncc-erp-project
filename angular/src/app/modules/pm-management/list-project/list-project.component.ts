...
export class ListProjectComponent extends PagedListingComponentBase<any> implements OnInit {
  ...
  public filterProjectType: any = this.APP_ENUM.FilterProjectType.All
  ...
  protected list(
    ...
    request.filterItems.forEach(item => {
      ...
      if (item.propertyName === "IsParentInvoiceIdExist") {
    ...
    if(checkFillterProject === false) {
      request.filterItems = this.AddFilterItem(request, "IsParentInvoiceIdExist", this.filterProjectType)
    }
    ...
    if(this.filterProjectType === 0 ) {
      request.filterItems = this.clearFilter(request, "IsParentInvoiceIdExist", "")
      checkFillterProject = true
    }
    ...
    this.listProjectService
      ...
      .subscribe((result: PagedResultResultDto) => {
        ...
        if (checkFillterProject == false) {
          request.filterItems = this.clearFilter(request, "fillterProject", "");
        }
        if(!checkFilterPM){
          request.filterItems = this.clearFilter(request, "pmId", "");
        }
