...
export class TimesheetDetailComponent extends PagedListingComponentBase<TimesheetDetailDto> implements OnInit {
  ...
  protected list(request: PagedRequestDto, pageNumber: number, finishedCallback: Function): void {
    ...
    this.timesheetProjectService.GetTimesheetDetail(this.timesheetId, request).pipe(catchError(this.timesheetProjectService.handleError)).pipe(finalize(()=>finishedCallback()))
      .subscribe((res) => {
        ...
        this.listTimesheetProject = this.listTimesheetProject.map(rs=>{
            if(rs.id == el.id){
              return el
            }
            else return rs
  ...
  Timesheets_TimesheetDetail_ActiveProject = PERMISSIONS_CONSTANT.Timesheets_TimesheetDetail_ActiveProject;
  Timesheets_TimesheetDetail_DeActivateProject = PERMISSIONS_CONSTANT.Timesheets_TimesheetDetail_DeActivateProject;
  ...
  exportTimesheetDetail(exportInvoiceMode) {
      const exportRequests = this.listExportInvoice.map(projectId => {
      let payloadDto = {
          timesheetId: this.timesheetId,
          projectIds: [projectId],
          mode: exportInvoiceMode
        };
        return this.timesheetProjectService.exportTimeSheetDetail(payloadDto);
      });
      forkJoin(exportRequests).subscribe(responses => {
  ...
  exportQuickTSDetail(item: any,exportInvoiceMode) {
    let invoiceExcelDto = {
      timesheetId: this.timesheetId,
      projectIds: [item.projectId],
      mode: exportInvoiceMode
    }
    this.timesheetProjectService.exportTimesheetDetail(invoiceExcelDto).pipe(catchError(this.timesheetProjectService.handleError)).subscribe(data => {
      const file = new Blob([this.s2ab(atob(data.result.base64))], {
        type: "application/vnd.ms-excel;charset=utf-8"
      });
      FileSaver.saveAs(file, data.result.fileName);
  ...
  addProjectToExport(event, item) {
    ...
    if (!event.checked) {
      ...
      this.listTimesheetProject.splice(index, 1);
      this.checkExportInvoice(this.listTimesheetProject)
    }
    else {
      ...
      this.listTimesheetProject.push(item);
      this.checkExportInvoice(this.listTimesheetProject)
    }
  ...
  checkExportInvoice(listTimesheetProject) {
    if (listTimesheetProject.length > 0) {
      ...
      let countCurrency = listTimesheetProject.reduce((r: any, a: any) => {
  ...
  selectActiveProject() {
    const show = this.dialog.open(ActiveTimesheetProjectComponent, {
      ...
      data: {
        type: 'Active',
        timesheetId: this.timesheetId,
        listTimesheetProject: this.listTimesheetProject.filter(res => !res.isActive)
      }
    })
