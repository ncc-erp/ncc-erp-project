...
namespace ProjectManagement.APIs.HRMv2
{
    public class Hrmv2AppService : ProjectManagementAppServiceBase
    {
        ...
        public Hrmv2AppService(IHttpContextAccessor httpContextAccessor, KomuService komuService,
            ResourceManager resourceManager, IBackgroundJobManager backgroundJob)
        {
        ...
        // HRMv2 call this api to update a user list 
        [AbpAllowAnonymous]
        [HttpPost]
        public async Task UpdateUsersFromHRM(List<CreateUpdateUserFromHRMV2Dto> inputs)
        {
            CheckSecurityCode();
            var emailInputs = inputs.Select(x => x.EmailAddress.ToLower().Trim()).ToList();
            var users = WorkScope.GetAll<User>()
                .Where(u => emailInputs.Contains(u.EmailAddress.Trim())).ToList();
            foreach (var input in inputs)
            {
                var user = users.Where(u => u.EmailAddress.Trim().Equals(input.EmailAddress.Trim(), StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
                var positionId = GetPositionIdByCode(input.PositionCode);
                var branch = await GetBranchByCode(input.BranchCode);
                if (user == null)
                {
                ...
                {
                    // only update properties changed, otherwise keep the original
                    if (input.EmailAddress.HasValue() && !user.EmailAddress.Equals(input.EmailAddress, StringComparison.OrdinalIgnoreCase))
                        user.EmailAddress = input.EmailAddress;
                    if (input.Surname.HasValue() && !user.Surname.Equals(input.Surname, StringComparison.OrdinalIgnoreCase))
                        user.Surname = input.Surname;
                    if (input.Name.HasValue() && !user.Name.Equals(input.Name, StringComparison.OrdinalIgnoreCase))
                        user.Name = input.Name;
                    if (user.PositionId != positionId)
                        user.PositionId = positionId;
                    if (input.EmailAddress.HasValue() && !user.UserName.Equals(input.EmailAddress, StringComparison.OrdinalIgnoreCase))
                        user.UserName = input.EmailAddress;
                    if (user.UserLevel != input.Level)
                        user.UserLevel = input.Level;
                    if (user.BranchId != branch.Id)
                        user.BranchId = branch.Id;
                    // employee status: working, pausing, maternity leave => IsActive = true
                    user.IsActive = input.EmployeeStatus != EmployeeStatus.Quit;
                    await WorkScope.UpdateAsync(user);
                }
            }
            // InActive all remain users in project
            var allUsers = WorkScope.GetAll<User>().Where(u => u.IsActive).ToList();
            var remainUsers = from r in allUsers
                              join i in inputs
                              on r.EmailAddress equals i.EmailAddress into t
                              from user in t.DefaultIfEmpty()
                              where user == default
                              select r.Id;
            await _backgroundJob.EnqueueAsync<InActiveUserBackgroundJob, InActiveUserBackgroundJobArgs>
           (new InActiveUserBackgroundJobArgs { Users = remainUsers });
        }
