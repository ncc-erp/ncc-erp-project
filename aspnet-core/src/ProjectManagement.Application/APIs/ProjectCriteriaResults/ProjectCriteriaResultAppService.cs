...
namespace ProjectManagement.APIs.ProjectCriteriaResults
{
    public class ProjectCriteriaResultAppService : ProjectManagementAppServiceBase
    {
        ...
        [HttpPut]
        public async Task<UpdateListProjectCriteriaDto> Updates(UpdateListProjectCriteriaDto input)
        {
            try
            {
                // get + update project criteria result list
                var projectCriteriaResults = WorkScope.GetAll<ProjectCriteriaResult>()
                    .Where(p => p.PMReportId == input.PMReportId && p.ProjectId == input.ProjectId).ToList();
                projectCriteriaResults.ForEach(p => { p.Status = input.Status; });
                await WorkScope.UpdateRangeAsync(projectCriteriaResults);

                // list empty or any criteria is not add to db: add all to ProjectCriteriaResult table
                var projectCriteriaResultIds = projectCriteriaResults.Select(p => p.ProjectCriteriaId).ToList();
                var projectCriterias = WorkScope.GetAll<ProjectCriteria>().Where(p => p.IsActive
                && !projectCriteriaResultIds.Contains(p.Id)).Select(pc => new ProjectCriteriaResult
                {
                    PMReportId = input.PMReportId,
                    Status = input.Status,
                    ProjectCriteriaId = pc.Id,
                    ProjectId = input.ProjectId
                });
                await WorkScope.InsertRangeAsync(projectCriterias);

                // update pm report project health
                await UpdateHealth(input.PMReportId, input.ProjectId, CommonUtil.GetProjectHealthByString(input.Status.ToString()));
