...
namespace ProjectManagement.APIs.Projects
{
    ...
    public class ProjectAppService : ProjectManagementAppServiceBase
    {
        ...
        [HttpGet]
        public async Task<bool> GetProjectInfoCheckbyId(long projectId)
        {
            var query = WorkScope.GetAll<Project>()
                ...
                .Select(x => new ProjectInfoDto
                {
                    ...
                    ParentInvoices = x.ParentInvoiceId != null
                ? WorkScope.GetAll<Project>().Where(u => u.Id == x.ParentInvoiceId)
                    .Select(u => new SubProjectInvoiceDto
                    {
                        Id = u.Id,
                        Name = u.Name,
                        Code = u.Code,
                    ...
                    .ToList()
                : WorkScope.GetAll<Project>().Where(u => u.ParentInvoiceId == x.Id)
                    .Select(u => new SubProjectInvoiceDto
                    {
                        Id = u.Id,
                        Name = u.Name,
            ...
            if (result != null)
            {
                if (result.ParentInvoiceId != null)
                {
                    return false;
                }
                else
                {
                    return result.ParentInvoices != null && result.ParentInvoices.Any();
                }
            }
