...
namespace ProjectManagement.APIs.TimeSheetProjectBills
{
    ...
    public class TimeSheetProjectBillAppService : ProjectManagementAppServiceBase
    {
        ...
        {
            // if timesheet is closed -> throw message
            var timeSheetProject = WorkScope.GetAll<TimesheetProject>().Include(tp => tp.Timesheet)
                .Where(tp => tp.TimesheetId == timesheetId && tp.ProjectId == projectId).FirstOrDefault();
            if (timeSheetProject != null && timeSheetProject.IsActive == false && timeSheetProject.Timesheet.IsActive == false)
                throw new UserFriendlyException("This timesheet project was closed!");
            ...
            var query = WorkScope.GetAll<TimesheetProjectBill>()
                ...
                .OrderBy(x => x.User.EmailAddress)
                         .Select(x => new GetTimeSheetProjectBillDto
                         {
                             ...
                             BillRole = x.BillRole,
                             BillRate = x.BillRate,
                             BillRateDisplay = isViewRate ? x.BillRate : -1,
